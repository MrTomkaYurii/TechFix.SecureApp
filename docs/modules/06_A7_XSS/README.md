...
Антипатерн: Пошуковий запит повертається клієнту у форматі HTML без контекстного екранування. Браузер інтерпретує рядок <script> як код JavaScript (Reflected XSS, CWE-79).
...
Remediation: Застосування контекстного енкодера HtmlEncoder.Default.Encode() або повернення строго типізованого JSON DTO.
...
Антипатерн: Відгуки клієнтів зберігаються в базі даних без фільтрації та транслюються іншим користувачам у відкритому вигляді (Stored XSS).
...
Remediation: Санітизація контенту під час збереження та екранування спеціальних символів під час читання списку відгуків.
...
Антипатерн: Клієнтський скрипт підставляє неперевірений параметр URL у sink-властивість window.location.href, дозволяючи виконати псевдо-схему javascript:alert() (DOM XSS).
...
Remediation: Сувора валідація протоколу URL за білим списком (лише http: та https:) та відхилення схем javascript: і data:.
...
Антипатерн: Сесійні cookie виставляються без прапорця HttpOnly, відкриваючи можливість їх викрадення через document.cookie за допомогою впровадженого XSS-скрипта.
...
Remediation: Обов'язкове встановлення прапорців HttpOnly = true, Secure = true та SameSite = SameSiteMode.Strict.
