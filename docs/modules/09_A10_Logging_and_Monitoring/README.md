...
Антипатерн: Несанітизовані вхідні дані записуються безпосередньо у текстовий файл журналу через розрив рядка (\r\n), що дозволяє підробити системні записи та приховати сліди злому (CWE-117).
...
Remediation: Санітизація спецсимволів розриву рядка та використання структурованого логування у форматі JSON (Structured JSON Logging).
...
Антипатерн: Здійснення фінансових дій (повернення коштів) без реєстрації в захищеному журналі аудиту (CWE-778). Адміністратор або зловмисник може вивести гроші безслідно.
...
Remediation: Обов'язкове створення незмінного запису аудиту з криптографічним хешем SHA256, фіксацією часу UTC та IP клієнта.
...
Антипатерн: Численні помилки входу не фіксуються в системі аналітики аномалій; відсутня реакція на атаку перебором паролів.
...
Remediation: Порогове виявлення підозрілої активності (понад 3 невдалі спроби за 60 секунд) та генерація високопріоритетного SIEM-алерту.
...
Remediation: Надання захищеного API для експорту та аналізу структурованих подій телеметрії безпеки.
